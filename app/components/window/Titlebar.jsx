...
export const Titlebar = () => {
    ...
    return (<div className={`window-titlebar ${wcontext?.platform ? `platform-${wcontext.platform}` : ''}`}>
      {wcontext?.platform !== 'darwin' && (<div className="window-titlebar-icon">
          <img src={icon}/>
        </div>)}

      <div className="window-titlebar-title" {...(titleCentered && { 'data-centered': true })} style={{ visibility: menusVisible ? 'hidden' : 'visible' }}>
        {title}
      </div>
...
const TitlebarControls = () => {
    const { window: wcontext } = useWindowContext();
    return (<div className="window-titlebar-controls">
...
const TitlebarControlButton = ({ svgPath, label }) => {
    ...
    return (<div aria-label={label} className="titlebar-controlButton" onClick={handleAction}>
