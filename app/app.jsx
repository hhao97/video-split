...
export default function App() {
    ...
    return (<main className="video-tool">
      ...
      <section className="workspace">
        <div className="panel inputs-panel">
          ...
          <Field label="片段输出目录" value={clipsOutputDir} action={() => chooseFolder(setClipsOutputDir)} buttonText="选择目录"/>
          <Field label="合并输出目录" value={mergedOutputDir} action={() => chooseFolder(setMergedOutputDir)} buttonText="选择目录"/>
        </div>

        <div className="panel settings-panel">
          <label className="control">
            <span>每段时长（秒）</span>
            <input type="number" min="1" max="3600" value={segmentSeconds} onChange={(event) => setSegmentSeconds(Number(event.target.value))}/>
          </label>
          ...
          <div className="control">
            ...
            ['center', '居中'],
            ['bottom', '底部'],
        ].map(([value, label]) => (<button key={value} className={textPosition === value ? 'active' : ''} type="button" onClick={() => setTextPosition(value)}>
                  {label}
                </button>))}
            </div>
          </div>
      ...
      {(error || result) && (<section className={error ? 'message error' : 'message success'}>
          {error ? (error) : (<>
...
function Field({ label, value, action, buttonText, }) {
    return (<label className="control file-field">
      <span>{label}</span>
      <div>
